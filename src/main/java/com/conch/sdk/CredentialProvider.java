// COMPILE-TIME STUB — mirrors the Conch SDK's CredentialProvider interface.
// At runtime, Conch's vault plugin provides the real implementation via its
// own classloader. This file is excluded from the plugin jar (see
// build.gradle.kts `tasks.jar { exclude("com/conch/sdk/**") }`) so there
// is only one CredentialProvider class at runtime — the one Conch provides.
// Do NOT modify this file unless the upstream Conch SDK changes; if it does,
// re-copy from conch_workbench/sdk/src/com/conch/sdk/CredentialProvider.java.
